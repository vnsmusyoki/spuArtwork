...
";
    ...
    $checkemail = "SELECT * FROM `agent` WHERE `agent_email` = '$email'";
    ...
    toastr.error('Username already exists. Please confirm your email  again .');
            ...
            $sql = "INSERT INTO `agent` (`agent_full_names`,`agent_email`, `agent_phone_number`, `agent_gender`, `agent_home_address`) VALUES ('$fullnames', '$email', '$phone_number', '$gender', '$description')";
            $result = mysqli_query($conn, $sql);
            $lastid = mysqli_insert_id($conn);
            $password = md5($password);
            $addstaff = "INSERT INTO `login`(`login_username`, `login_rank`, `login_admin_id`, `login_agent_id`,`login_user_id`, `login_password`) VALUES ('$username','agent',null,'$lastid',null,'$password')";
